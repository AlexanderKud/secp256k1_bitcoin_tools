...
P = secp256k1.publickey_to_point(puzzle_pubkey)
slice_width = 6
...
while step < (end_range - start_range):
    cpub_slice = secp256k1.point_to_cpub(S)[2:slice_width]
    if cpub_slice in baby_table:
        b = baby_table.index(cpub_slice)
        k = start_range + step + b
        if secp256k1.point_to_cpub(secp256k1.scalar_multiplication(k)) == puzzle_pubkey:
